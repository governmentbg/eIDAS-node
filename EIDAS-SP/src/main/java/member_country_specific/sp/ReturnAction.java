...
public class ReturnAction extends ActionSupport implements ServletRequestAware, ServletResponseAware {
	...
	public String execute() {
		...
		eavtReturnUrl = configs.getProperty(Constants.EAVT_RETURN_URL);
		
		//comment it in future
		eavtReturnUrl = eavtReturnUrl == null ? null : "http://localhost:8090/eAuthenticator/loginEidas.seam;jsessionid=";
		...
		try {
			Response json = new SimpleProtocolProcess().convertFromJson(new StringReader(smsspUnencryptedResponseJSON),
					Response.class);

			this.sessionId = json.getInResponseTo();
	...
	public String populate() throws JAXBException {
		...
		if ((jsonResponseStatus != null && "failure".equals(jsonResponseStatus))) {
			throw new ApplicationSpecificServiceException("Smssp Response is fail",
					jsonResponse.getStatus().getStatusMessage());
		} else {
	...
	public Map<AttributeDefinition<?>, Set<String>> mapAttributesSimpleResponseToLightResponse(Response smsspResponse) {
		...
		for (Attribute attribute : attributes) {
			...
			ImmutableSortedSet<AttributeDefinition<?>> attributeDefinition = coreAttributeRegistry
					.getByFriendlyName(name);

			ImmutableSortedSet<AttributeDefinition<?>> additionalAttributeDefinition = coreAdditionalAttributeRegistry
					.getByFriendlyName(name);

			AttributeDefinition<?> attributeDefinitionOut = attributeDefinition.isEmpty()
					? additionalAttributeDefinition.first() : attributeDefinition.first();
	...
	public String getReturnUrl() {
		return  eavtReturnUrl + sessionId;
	}
